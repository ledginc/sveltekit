<script lang="ts">
  ...
  import { addObjectToProcess, createProcess, process, updateProcess, updateSection } from "./index.svelte";
        ...
        dropZones.forEach((dropZone) => {
          ...
          dropZone.addEventListener("drop", (e) => {
            ...
            if (event && event.dataTransfer) {
              ...
              addObjectToProcess(category, type, sectionId as string);
            }
          });
        });
          ...
          <div class="box-border flex flex-col items-center bg-base-200 p-4 w-full math gap-10 overflow-y-auto">
            {#each currentProcess.sections as section}
              <div class="h-auto w-4/5 bg-base-100 box-border p-3 rounded-lg items-center flex flex-col gap-3" data-containertype="section" data-sectionid={section.id}>
                <input class="input input-ghost w-full max-w-xs text-center" value={section.name} onblur={(e) => updateSection({id: section.id, name: (e.target as HTMLInputElement).value})} />
