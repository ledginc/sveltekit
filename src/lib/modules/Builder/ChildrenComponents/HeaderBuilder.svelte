<script lang="ts">
  import { addSectionToProcess } from "../index.svelte";
  import { process } from "../index.svelte";

  let name: string = "";
</script>

<div class="w-full h-12 border-b p-1 box-border flex justify-between items-center">
  <h2 class="text-lg font-semibold box-border">Création du processus <span class="font-bold">{$process?.name}</span></h2>
  {#if $process}
    <input class="input input-ghost w-full max-w-xs text-center" bind:value={$process.name} />
  {/if}
  <button class="btn btn-primary btn-sm" onclick={addSectionToProcess}>
