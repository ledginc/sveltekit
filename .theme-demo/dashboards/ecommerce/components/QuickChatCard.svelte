<script lang="ts">
    import QuickChatItem, { type IQuickChatItem } from "./QuickChatItem.svelte";
...
<div class="card bg-base-100 shadow">
    <div class="card-body pb-3">
        <div class="flex items-center gap-3">
            <span class="iconify lucide--messages-square size-4.5" />
            <span class="font-medium">Quick Chat</span>
            <a href="/apps/chat" class="btn btn-outline btn-sm border-base-300 ms-auto"> Go To Chat </a>
        </div>
        <div class="-mx-2 mt-2 space-y-0.5">
            {#each quickChats as chat, index (index)}
                <QuickChatItem {...chat} />
            {/each}
        </div>
    </div>
